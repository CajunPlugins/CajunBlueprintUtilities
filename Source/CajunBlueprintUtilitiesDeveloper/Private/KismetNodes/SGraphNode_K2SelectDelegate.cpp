...
{
	if (const auto SelectFunctionWidgetPtr = FunctionOptionComboBox.Pin(); SelectFunctionWidgetPtr.IsValid())
	{
...
void SGraphNode_K2SelectDelegate::CreateBelowPinControls(TSharedPtr<SVerticalBox> MainBox)
{
	if (const auto Node = Cast<IK2Interface_SelectDelegate>(GraphNode))
	{
		...
		if (const auto ScopeClass = Node->GetScopeClass(); FunctionSignature != nullptr && ScopeClass != nullptr)
		{
			...
			{
				if (const auto Func = *It; Func && IsFunctionCompatible(Func) && UEdGraphSchema_K2::FunctionCanBeUsedInDelegate(Func))
				{
			...
			ClassFunctions.Sort([](const FFunctionItemData& A, const FFunctionItemData& B) { return A.Description.CompareTo(B.Description) < 0; });
			...
			const TSharedRef<SSearchableComboBox> SelectFunctionWidgetRef = SNew(SSearchableComboBox)
	...
	if (FunctionItemData.IsValid())
	{
		if (const auto Node = Cast<UK2Node>(GraphNode))
		{
			...
			if (FunctionItemData == CreateMatchingFunctionData)
			{
				...
				const auto NewGraph = FBlueprintEditorUtils::CreateNewGraph(NodeBP, NewFuncName, SourceGraph->GetClass(),
				                                                            SourceGraph->GetSchema() ? SourceGraph->GetSchema()->GetClass() : GetDefault<UEdGraphSchema_K2>()->GetClass());
				if (NewGraph != nullptr)
				{
