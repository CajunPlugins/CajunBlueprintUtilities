...
bool UK2Node_BindDelegate::IsValid(FString* OutMsg, bool bDontUseSkeletalClassForSelf) const
{
	...
	if (ScopeClass == nullptr)
	{
		if (OutMsg != nullptr)
		{
			...
			*OutMsg = FText::Format(
				LOCTEXT("Class_not_found_fmt", "Unable to determine context for the selected function/event: '{0}' - make sure the target '{1}' pin is properly set up."),
	...
	{
		if (OutMsg != nullptr)
			*OutMsg = FText::Format(LOCTEXT("Function_not_found_fmt", "Unable to find the selected function/event: '{0}' - has it been deleted?"), FText::FromString(FunctionName.ToString())).
				ToString();
	...
	if (!Signature->IsSignatureCompatibleWith(FoundFunction))
	{
		if (OutMsg != nullptr)
			*OutMsg = FText::Format(
				LOCTEXT("Function_not_compatible_fmt", "The function/event '{0}' does not match the necessary signature - has the delegate or function/event changed?"),
...
bool UK2Node_BindDelegate::HandleAnyChangeWithoutNotifying()
{
	...
	const auto bIsSelfScope = (SelfScopeClass != nullptr) && (ParentClass != nullptr) && ((SelfScopeClass->IsChildOf(ParentClass)) || (SelfScopeClass->ClassGeneratedBy == ParentClass->
		ClassGeneratedBy));
...
void UK2Node_BindDelegate::UpdateNodeTitle()
{
	if (SelectedDelegateName.IsNone())
	{
		CachedNodeTitle.SetCachedText(FText::FromString(TEXT("Bind Delegate")), this);
	}
	else
	{
		FText Title = FText::Format(FTextFormat::FromString(TEXT("Bind to {0} Delegate")), FText::FromName(SelectedDelegateName));
		if (!Title.EqualTo(CachedNodeTitle.GetCachedText()))
		{
...
{
	return CachedNodeTitle.GetCachedText().IsEmpty() ? LOCTEXT("NodeName", "Bind Delegate") : CachedNodeTitle.GetCachedText();
}
...
{
	CajunK2Utilities::DefaultGetMenuActions(this, ActionRegistrar);
}
