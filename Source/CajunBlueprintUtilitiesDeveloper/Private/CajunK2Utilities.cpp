...
void CajunK2Utilities::MovePinLinksOrCopyDefaults( FKismetCompilerContext &CompilerContext, UEdGraphPin *Source, UEdGraphPin *Dest )
{
...
void CajunK2Utilities::RefreshAllowedConnections( const UK2Node *K2Node, UEdGraphPin *Pin )
{
...
UEdGraphPin* CajunK2Utilities::GetInputPinLink( UEdGraphPin *Pin )
{
...
void CajunK2Utilities::SetPinToolTip( UEdGraphPin *Pin, const FText &PinDescription )
{
...
void CajunK2Utilities::DefaultGetMenuActions( const UK2Node *Node, FBlueprintActionDatabaseRegistrar& ActionRegistrar )
{
...
UK2Node_CustomEvent* CajunK2Utilities::CreateCustomEvent( FKismetCompilerContext &CompilerContext, UEdGraphPin *SourcePin, UEdGraph *SourceGraph, UK2Node *Node,
...
TArray< UEdGraphPin* > CajunK2Utilities::CreateFunctionPins( UK2Node *Node, const UFunction *Signature, EEdGraphPinDirection Dir, bool bMakeAdvanced, const FGetPinName &GetPinName, const FGetPinText &GetPinTooltip )
{
...
void CajunK2Utilities::CreateEventDispatcherPins( const UClass *Class, UK2Node *Node, TArray< UEdGraphPin* > *OutDispatcherPins, bool bMakeAdvanced, const TArray< FName > &IgnoreDispatchers )
{
	check( Class != nullptr );
	check( Node != nullptr );
...
void CajunK2Utilities::ExpandFunctionPins( const UK2Node *Node, const UFunction *Signature, EEdGraphPinDirection Dir, const FGetPinName &GetPinName, const FDoPinExpansion &DoPinExpansion )
{
	if (!ensureAlways( Node != nullptr ))
		return;
	if (!ensureAlways( Signature != nullptr ))
...
UEdGraphPin* CajunK2Utilities::ExpandDispatcherPins( FKismetCompilerContext &CompilerContext, UEdGraph *SourceGraph, UK2Node *Node, UEdGraphPin *ExecPin, const UClass *Class, UEdGraphPin *InstancePin, const TFunction< bool( UEdGraphPin* ) > &IsGeneratedPin )
{
...
void CajunK2Utilities::ReorderPin( UK2Node *Node, UEdGraphPin *Pin, int NewIndex )
{
	check( Node != nullptr );
	check( Pin != nullptr );
...
FSlateIcon CajunK2Utilities::GetFunctionIconAndTint( FLinearColor& OutColor )
{
...
FSlateIcon CajunK2Utilities::GetPureFunctionIconAndTint( FLinearColor& OutColor )
{
...
void CajunK2Utilities::HandleGraphChange( const UK2Node *Node, bool bMarkDirty )
{
	check( Node != nullptr );
